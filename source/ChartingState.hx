	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!typingShit.hasFocus)
		{
			...
			if (!FlxG.keys.pressed.CONTROL)
			{
				if (#if (mobileC || mobileCweb) virtualPad.buttonRight.justPressed
					|| #end FlxG.keys.justPressed.RIGHT || FlxG.keys.justPressed.D)
					changeSection(curSection + shiftThing);
				if (#if (mobileC || mobileCweb) virtualPad.buttonLeft.justPressed
					|| #end FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.A)
					changeSection(curSection - shiftThing);
			}
			...
			if (!FlxG.keys.pressed.SHIFT)
			{
				if (#if (mobileC || mobileCweb) virtualPad.buttonUp.pressed
					|| virtualPad.buttonDown.pressed
					|| #end FlxG.keys.pressed.W || FlxG.keys.pressed.S)
				{
					FlxG.sound.music.pause();
					vocals.pause();
					claps.splice(0, claps.length);
			...
			{
				if (#if (mobileC || mobileCweb) virtualPad.buttonUp.justPressed
					|| virtualPad.buttonDown.justPressed
					|| #end FlxG.keys.justPressed.W || FlxG.keys.justPressed.S)
				{
					FlxG.sound.music.pause();
					vocals.pause();
					...
					if (#if (mobileC || mobileCweb) virtualPad.buttonUp.justPressed || #end FlxG.keys.justPressed.W)
					{
						FlxG.sound.music.time -= daTime;
					}
					else
